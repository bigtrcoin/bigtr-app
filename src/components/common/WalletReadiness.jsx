...
const WalletReadiness = ({ account, amount, onChange }) => {
  ...
  // USDT still sitting in the signed-in wallet counts too: the purchase moves
  // what it needs across automatically (one confirmation in that wallet).
  const spendable = usdt + (signerEnabled ? signerUsdt : 0);
  const topUp = want > 0 ? Math.max(0, Math.min(want, spendable) - usdt) : 0;
  const topUpNeeded = signerEnabled && topUp > 0;
  const needBnb =
    !loading &&
    ((!sponsored && bnb < MIN_GAS_BNB) || (topUpNeeded && signerBnb < MIN_GAS_BNB));
  const needUsdt = !loading && (want > 0 ? spendable < want : spendable <= 0);
  const missingUsdt = want > 0 ? Math.max(0, want - spendable) : 0;
  ...
  const state = useMemo(() => {
    ...
    if (needBnb)
      return {
        ok: false,
        label: "Add BNB for fees to continue",
        reason: topUpNeeded
          ? `Your USDT is in the wallet you signed in with, and moving it needs a little BNB there. Send at least ${RECOMMENDED_GAS_BNB} BNB to that wallet, or withdraw USDT from an exchange straight to the address shown here.`
          : `Your wallet has no BNB for network fees. Send at least ${RECOMMENDED_GAS_BNB} BNB to it first.`,
      };
    return { ok: true, label: "Buy Now", reason: "" };
  }, [enabled, loading, needUsdt, needBnb, topUpNeeded]);
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1">
        ...
        {!loading && (
          <p className="font-chakrapetch text-[13px] text-secondary-80">
            <span className={needUsdt ? "text-[#F2B43C] font-bold" : "text-secondary font-bold"}>{fmt(spendable, 2)} USDT</span>
            {sponsored ? (
        ...
        <div className="mt-3 font-chakrapetch text-[13px] text-secondary-80 leading-relaxed">
          {needUsdt && (
            <p className="mb-1.5">
              ...
              {spendable <= 0
                ? "you have no USDT yet."
                : `you have ${fmt(spendable, 2)} USDT but this purchase needs ${fmt(want, 2)} USDT (${fmt(missingUsdt, 2)} more).`}
            </p>
          )}
          {needBnb && (
            <p className="mb-1.5">
          ...
          <p className="mt-3 text-[12px] text-secondary-70">
            {SMART_ACCOUNT_FOR_ALL
              ? "This is your BigTR pre-sale address. USDT already in the wallet you signed in with does not need to be sent by hand — the purchase moves what it needs when you press Buy Now."
